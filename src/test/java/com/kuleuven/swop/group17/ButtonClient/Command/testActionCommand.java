...
public class testActionCommand {
	...
	public void setUp() throws Exception {
		when(gameController.performAction(action)).thenReturn(snapshot);
	}
	...
	public void testActionCommandConstructorGameControllerNull() {
		...
		Command command = new ActionCommand(null,action);
		
	}
	
	@Test
	public void testActionCommandConstructorActionNull() {
		String excMessage = "Action may not be null.";
		exceptionRule.expect(NullPointerException.class);
		exceptionRule.expectMessage(excMessage);
		
		Command command = new ActionCommand(gameController,null);
	...
	public void testExecute() {
		...
		command.execute();
		verify(gameController).performAction(action);
		try {
			Field f = ActionCommand.class.getDeclaredField("snapshot");
